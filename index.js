   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +24165506084")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 😍\nFor example: +24165506084 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
...
//------------------------------------------------------
    XeonBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
            ...
            await delay(1000 * 2) 
             const xeonses = await  XeonBotInc.sendMessage(loadingBotInc.user.id, { document: sessionXeon, mimetype: `application/json`, fileName: `creds.json` })
             await XeonBotInc.sendMessage(XeonBotInc.user.id, { text: `⚠️Do not share this file with anybody⚠️\n
┌─❖
│ guy man youpii 😽
└┬❖  
┌┤✑  Thanks for using loading-PairCode
│└────────────┈ ⳹        
│©2020-2023 loading BotInc 
└─────────────────┈ ⳹\n\n ` }, {quoted: xeonses});
              await delay(1000 * 2) 
              process.exit(0)
        }
